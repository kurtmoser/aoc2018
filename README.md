...
My initial solution was to place each star into its own constellation and then start merging constellations two at a time until constallations list wasn't decreasing in length any more (starting always from the beginning of the constallations list, i.e. during the last merges we made lots of unnecessary checking before reaching merge candidates). While this approach worked and had reasonable runtime it could be improved. So instead of original constellation creation (one star per constellation) I instead placed immediately as many stars as possible into constallations with others. After that I moved through (already shorter than original) constellations list from end to beginning with 2 runners. At each point check if constellations are connected and if they are merge backrunner's constellation into frontrunner's one and delete backrunner constellation. Then move runners back to the end of the list. Once backrunner reaches beginning of constellations list we have all of our constellations merged.
